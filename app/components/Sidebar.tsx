...
export default function Sidebar() {
    ...
    const handleIntersection = (entries: IntersectionObserverEntry[]): void => {
      ...
      if (bestEntry) {
        const index = navItems.findIndex(
          (item: NavItem) => item.id === (bestEntry as IntersectionObserverEntry).target.id
        );
  ...
  const scrollToSection = (id: string, index: number): void => {
    ...
    if (el) {
      ...
      const elementPosition: number = el.offsetTop - offset;
  ...
  return (
    <>
        ...
        {isOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-50 flex h-screen w-[260px] flex-col justify-between bg-[#07111d] px-7 py-10 text-white lg:hidden"
            >
              <button
                onClick={(): void => setIsOpen(false)}
                ...
                <nav className="space-y-4">
                  {navItems.map((item: NavItem, i: number) => (
                    <motion.button
                      ...
                      className={`block w-full text-left text-base transition-all duration-300 hover:translate-x-1 hover:text-cyan-400 ${
                        activeIndex === i ? "text-cyan-400" : "text-white/80"
                      }`}
                    >
                      {item.label}
                    </motion.button>
      ...
      {/* ─── DESKTOP SIDEBAR ─── */}
      <aside className="fixed left-0 top-0 z-50 hidden h-screen w-[280px] flex-col justify-between bg-[#07111d] px-8 py-10 text-white border-r border-white/5 lg:flex">
        <div>
          ...
          <nav className="space-y-5 text-lg">
            {navItems.map((item: NavItem, i: number) => (
              <button
                key={item.id}
                onClick={(): void => scrollToSection(item.id, i)}
                className={`block w-full text-left transition-all duration-300 hover:translate-x-1 hover:text-cyan-400 ${
                  activeIndex === i ? "text-cyan-400" : "text-white/90"
                }`}
              >
                {item.label}
              </button>
